...
CompleteRead(
    ...
    _In_ DWORD dwBytesCount,
    _In_ PCALLBACK_DATA pData
)
/**
Routine Description:
...
Arguments:
    ...
    pData - The pointer to the CALLBACK_DATA structure passed to the
...
Arguments:
    ...
    pInParameters - Pointer to a PWDS_TRANSPORTPROVIDER_INIT_PARAMS structure 
...
--*/ 
{
    ...
    // to tie up a file indefinitely, preventing any sort of maintanence on that
    // file.
    //
...
Arguments:
    ...
    hContent - Handle to an open content stream to be read. This is the handle
               returned by the WdsTransportProviderOpenContent callback.
    pBuffer - Pointer to location to receive read content.
    ulBytesToRead - The size in bytes of the buffer at the location specified
                    by the pBuffer parameter.
    pContentOffset - The offset into the content stream specified by hContent
                     from which to start reading.
    pvUserData - User specified data passed to the callback function.
