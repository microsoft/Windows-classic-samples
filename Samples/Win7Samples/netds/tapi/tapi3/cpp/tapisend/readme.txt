...
Before building the TAPISend sample application you need to 
	...
	-Set the build configuration of the BaseClasses sample to "Debug Unicode" or "Release Unicode" 
		using Visdual Studio's build configuration manager to build UNICDE libraries
	-run the setenv.bat file in the Platform SDK root dir
	-set the DXSDK_DIR enviroment variable to point to the DirectX SDK install root(if not automatically set).
	 You can do this in the setenv.bat in the Microsoft Platform SDK root directory
		-e.g. in setenv.bat add the following line:
