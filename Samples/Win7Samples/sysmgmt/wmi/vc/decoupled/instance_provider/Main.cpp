...
/******************************************************************************
 *
 *	Name:	RegisterDecoupledInstance
 *
 *	
 *  Description:
 *
 *			Register a decoupled instance provider object with WMI.
 *			We pass in an implementation of IWbemServices so 
 *			that WMI can instance retrieval requests.	
 *
 *****************************************************************************/
...
/*
 *	Create the registrar for the instance provider, so that we can register with WMI.
 */
	...
	{
/*
 *	Create our implementation instance of the instance provider
 */

		CProvider_IWbemServices *t_Service = new CProvider_IWbemServices () ;
		if ( t_Service )
		{
			t_Service->AddRef () ;
...
/******************************************************************************
 *
 *	Name:		Process_DecoupledInstance
 *
 *	
 *  Description:
 *
...
HRESULT Process_DecoupledInstance ()
{
	IWbemDecoupledRegistrar *t_InstanceRegistrar = NULL ;

	HRESULT t_Result = RegisterDecoupledInstance ( t_InstanceRegistrar ) ;
	if ( SUCCEEDED ( t_Result ) )
	{
		WindowsDispatch () ;
	}
...
HRESULT Process ()
{
	...
	if ( SUCCEEDED ( t_Result ) )
	{
		try 
		{
			t_Result = Process_DecoupledInstance () ;
		}
		catch ( ... )
		{
		}
