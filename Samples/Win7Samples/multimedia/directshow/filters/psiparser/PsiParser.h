...
// Releases the COM Object references safely
...
{
    friend class CPsiParserFilter;
    friend class CPATProcessor;
    friend class CPMTProcessor;

private:

    CPsiParserFilter *m_pFilter;        // The filter that owns us
...
public:
    ...
    CPsiParserInputPin(CPsiParserFilter *pTextOutFilter,
                  LPUNKNOWN pUnk,
                  CCritSec *pLock,
...
public:
    ...
    CPATProcessor(CPsiParserFilter *pParser,CPrograms * pPrograms, HRESULT *phr);
    ~CPATProcessor();
...
public:
    ...
    CPMTProcessor(CPsiParserFilter *pParser,CPrograms * pPrograms,HRESULT *phr);
    ~CPMTProcessor();
...
// This is the COM object that represents the parser filter

class CPsiParserFilter  :   public CBaseFilter,
                            public IMpeg2PsiParser,
...
public:
    ...
    CPsiParserFilter(LPUNKNOWN pUnk,HRESULT *phr);
    ~CPsiParserFilter();
    ...
    // from PAT
    STDMETHODIMP GetTransportStreamId       (WORD *pstreamId);
    STDMETHODIMP GetPatVersionNumber        (BYTE *pPatVersion);
    STDMETHODIMP GetCountOfPrograms         (int *pNumOfPrograms);
    STDMETHODIMP GetRecordProgramNumber     (DWORD dwIndex, WORD * pwVal);
    STDMETHODIMP GetRecordProgramMapPid     (DWORD dwIndex, WORD * pwVal);
    STDMETHODIMP FindRecordProgramMapPid    (WORD wProgramNumber, WORD * pwVal);

    // from PMT
    STDMETHODIMP GetPmtVersionNumber        (WORD wProgramNumber, BYTE *pPmtVersion);
    STDMETHODIMP GetCountOfElementaryStreams(WORD wProgramNumber, WORD *pwVal);
    STDMETHODIMP GetRecordStreamType        (WORD wProgramNumber,DWORD dwRecordIndex, BYTE *pbVal);
    STDMETHODIMP GetRecordElementaryPid     (WORD wProgramNumber,DWORD dwRecordIndex, WORD *pwVal);
