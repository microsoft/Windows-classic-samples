...
Arguments:
    ...
    hClientRequest  -   Handle to received request. Ignored.
    pPacket         -   Pointer to received data.
    uPacketLen      -   Length, in bytes, of pPacket. Ignored.
    pLocalAddress   -   Local Address on which this request was received.
                        For this implementation, the local address is ignored.
    pRemoteAddress  -   Remote Address of client who sent the request.
                        For this implementation, the remote address is ignored.
    pAction         -   [out] Contains the next action that should be taken by
                        WDS PXE.
    pContext        -   Context which was passed to WDSPXE when callback was 
                        registered. For this implementation, the context pointer
                        is ignored.
...
--*/
{
    ...
    UNREFERENCED_PARAMETER(hClientRequest);
    UNREFERENCED_PARAMETER(uPacketLen);
    UNREFERENCED_PARAMETER(pLocalAddress);
