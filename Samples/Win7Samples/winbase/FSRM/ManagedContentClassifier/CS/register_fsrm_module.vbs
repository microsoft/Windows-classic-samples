...
Wscript.Echo "" 
Wscript.Echo "REGISTER_FSRM_MODULE.VBS version 1.6.1 for Windows Server 2008"
...
Dim FsrmModuleName, ComponentProgID, ModuleDescription
If Args.Item(0) = "-register" Then 
	If Args.Count <> 3 Then PrintsUsage

	FsrmModuleName = Args.Item(1)
	ComponentProgID = Args.Item(2)
	ModuleDescription = "Fsrm Module with ProgID " & Args.Item(2) 

	UninstallModule
	InstallModule

        ' 
        ' TODO: Register with FSRM
        '
	Wscript.Quit 0
End If 
...
Sub PrintsUsage
	...
	Wscript.Echo "Usage:" 
	Wscript.Echo "" 
	Wscript.Echo " 1) Registering a FSRM Module as a COM+ application:" 
	Wscript.Echo "      CScript.exe " & Wscript.ScriptName & " -register <FsrmModuleName> <ComponentProgID>" 
	Wscript.Echo "" 
	Wscript.Echo " 2) Unregistering a COM+ application associated with a FSRM module:" 
	Wscript.Echo "      CScript.exe " & Wscript.ScriptName & " -unregister <FsrmModuleName>" 
	Wscript.Echo "" 
	Wscript.Quit 1
...
Sub InstallModule
	...
	Wscript.Echo "- Set app name = " & FsrmModuleName & " "
	app.Value("Name") = FsrmModuleName
	CheckObjectError 105, collApps, app

	Wscript.Echo "- Set app description = " & ModuleDescription & " "
	app.Value("Description") = ModuleDescription 
	...
	cat.CreateServiceForApplication FsrmModuleName, FsrmModuleName , "SERVICE_AUTO_START", "SERVICE_ERROR_NORMAL", "", ".\localsystem", "", 0
	CheckCollectionError 112, cat

	wscript.echo "- Add the DLL component"
	cat.ImportComponent FsrmModuleName, ComponentProgID
...
Sub UninstallModule
	...
	wscript.echo "- Search for " & FsrmModuleName & " application..."
	Dim numApps
	numApps = collApps.Count
	Dim i
	For i = numApps - 1 To 0 Step -1
	    If collApps.Item(i).Value("Name") = FsrmModuleName Then
	        collApps.Remove(i)
		CheckCollectionError 204, collApps
                WScript.echo "- Application " & FsrmModuleName & " removed!"
	    End If
	Next
